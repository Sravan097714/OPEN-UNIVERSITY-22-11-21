        ...
        area(content)
        {
            group(General)
            {
                ...
                field("Invoice Discount %"; "Invoice Discount %")
                {
                    ...
                    trigger OnValidate()
                    begin
                        if "Invoice Discount %" <> 0 then
                            TotalSalesLine."Inv. Discount Amount" := ("Invoice Discount %" / 100) * TempVATAmountLine."Line Amount"
                        else
                            TotalSalesLine."Inv. Discount Amount" := 0;
                ...
                field(InvDiscountAmount; TotalSalesLine."Inv. Discount Amount")
                {
                    ...
                    trigger OnValidate()
                    begin
                        if TotalSalesLine."Inv. Discount Amount" <> 0 then
                            "Invoice Discount %" := (TotalSalesLine."Inv. Discount Amount" * 100) / TempVATAmountLine."Line Amount"
                        else
                            "Invoice Discount %" := 0;
                        UpdateInvDiscAmount;
                    end;
